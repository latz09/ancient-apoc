export const gobekliTepe = {
	title: 'Gobekli Tepe: Proof of an ancient apocalypse?',
	category: 'example locations',
	pageLink: 'https://en.wikipedia.org/wiki/Ancient_Apocalypse',
	content: [
		{
			paragraph: {
				phraseToLink: '',
				link: '',
				text: 'Gobekli Tepe, an ancient site located in Turkey, has been the subject of much debate and research in recent years. The site, which dates back to around 10,800 BC, has been proposed as evidence for a massive ancient apocalypse that changed the course of human history.',
			},
			link: { text: 'Gobekli Tepe: The Worlds First Temple?', type: 'external', link: 'https://www.smithsonianmag.com/history/gobekli-tepe-the-worlds-first-temple-83613665/' },
		},
		{
			paragraph: {
				phraseToLink: '',
				link: '',
				text: 'According to the theory proposed by British author Graham Hancock, Gobekli Tepe is evidence of a single, massive impact event that occurred around 10,800 BC. This event, which he calls the "Gobekli Tepe Impact Event," was responsible for the collapse of ancient civilizations such as the Clovis culture in North America and the end of the last ice age.',
			},
			link: { text: '', type: 'external', link: '' },
		},
		{
			paragraph: {
				phraseToLink: 'Gobekli Tepe',
				link: 'https://en.wikipedia.org/wiki/G%C3%B6bekli_Tepe#:~:text=G%C3%B6bekli%20Tepe%20(Turkish%3A%20%5B%C9%9F%C5%93bec%CB%88li,Southeastern%20Anatolia%20Region%20of%20Turkey.',
				text: 'The ancient site of Gobekli Tepe in Turkey is one of the oldest known temple structures in the world, and its advanced architecture and intricate carvings have puzzled researchers for years. However, recent discoveries at the site have provided new evidence to support Hancocks theory. Researchers have uncovered evidence of an advanced civilization that existed before the collapse caused by this catastrophic event.',
			},
			link: { text: '', type: 'external', link: '' },
		},
		{
			paragraph: {
				phraseToLink: '',
				link: '',
				text: 'The excavations at Gobekli Tepe have revealed a complex network of stone circles and pillars, some of which are decorated with intricate carvings of animals and symbols. Radiocarbon dating has placed the construction of these structures at around 10,800 BC, which coincides with the proposed date of the impact event. This suggests that an advanced civilization existed before the collapse caused by this event.',
			},
			link: { text: 'The Stone Circles of Gobekli Tepe: Challenging Our Understanding of Human History', type: 'internal', link: '/example-locations/articles/gobekliTepeCircles' },
		},
		{
			paragraph: {
				phraseToLink: '',
				link: '',
				text: 'Critics argue that there is not enough evidence to support the idea of a single, massive impact event. They argue that the collapse of ancient civilizations and the end of the last ice age is more likely to have been caused by a combination of factors, such as climate change and human conflict. However, the recent discoveries at Gobekli Tepe have provided new evidence to support Hancocks theory and have sparked renewed interest in the possibility of a massive impact event in human history.',
			},
			link: { text: 'A Catastrophic Event Described in Ancient Cultures Worldwide', type: 'internal', link: '/ancient-stories/articles/greatFloods' },
		},
	],
};
